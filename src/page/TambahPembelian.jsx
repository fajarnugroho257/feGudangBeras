  ...
  const fetchBarangByType = useCallback(
    async (tipe) => {
      ...
      try {
        ...
        if (
          response.status === 200 &&
          Array.isArray(response.data.dataBarang)
        ) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (isConfirmed) {
      ...
      const invalidItem = inputFields.find(
        (item) =>
          !item.selectedBarang ||
          (item.selectedBarang.value === "new" && !item.barang_nama),
      );
      if (invalidItem) {
        setIsSubmitting(false);
        alert(
          "Setiap item harus memiliki Barang yang dipilih, dan jika Barang Baru, nama harus diisi.",
        );
        return;
      }
      ...
      try {
        const data = {
          suplier_id:
            selectedSupplier.value === "new" ? null : selectedSupplier.value,
        ...
        const sanitizedFields = inputFields.map((item) => ({
          ...item,
          barang_id:
            item.selectedBarang?.value === "new" ? null : item.barang_id,
        }));
        let params = {
        ...
        if (btnValue === "simcetak") {
          ...
          const pembelianId = response.data.pembelianId;
          try {
            const response = await api.get(`/detail-Pembelian/${pembelianId}`, {
              headers: {
            ...
            const header =
              ...
              `Supplier    : ${supplier}\n` +
              `Tanggal     : ${tanggal}\n` +
              `DP          : ${formatRupiah(parseInt(dataSuplier.dp))}\n` +
            ...
            <div className="md:hidden space-y-4 mb-6">
              ...
              {inputFields.map((field, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative flex flex-col gap-3"
                  >
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="col-span-2">
                          ...
                          <Select
                            ...
                            onMenuOpen={() =>
                              fetchBarangByType(field.barang_tipe)
                            }
                            onChange={(selected) => {
                              const values = [...inputFields];
                              values[index].selectedBarang = selected;
                              values[index].barang_id =
                                selected && selected.value !== "new"
                                  ? selected.value
                                  : null;
                              setInputFields(values);
                            }}
                      ...
                      {/* Input Barang Baru */}
                      {field.selectedBarang &&
                        field.selectedBarang.value === "new" && (
                          <div>
                            <input
                              name="barang_nama"
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-1">
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-red-400 uppercase mb-1 block text-center">
                          Potongan (Kg)
                        </label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-green-600 uppercase mb-1 block text-center">
                          Bersih (Kg)
                        </label>
                        <input
                    ...
                    <div className="flex justify-between items-center border-t border-gray-100 pt-3 mt-1">
                      <div className="flex bg-gray-100 rounded-lg p-1 border border-gray-200 w-1/2">
                        <label
                          className={`flex-1 flex items-center justify-center cursor-pointer py-1.5 rounded-md transition-all ${field.pembayaran === "cash" ? "bg-teal-600 text-white shadow-sm" : "text-gray-500"}`}
                        >
                          <input
                            type="radio"
                            className="hidden"
                            ...
                            onChange={() =>
                              handleInputChange(index, {
                                target: { name: "pembayaran", value: "cash" },
                              })
                            }
                        ...
                        <label
                          className={`flex-1 flex items-center justify-center cursor-pointer py-1.5 rounded-md transition-all ${field.pembayaran === "hutang" ? "bg-red-500 text-white shadow-sm" : "text-gray-500"}`}
                        >
                          <input
                            type="radio"
                            className="hidden"
                            ...
                            onChange={() =>
                              handleInputChange(index, {
                                target: { name: "pembayaran", value: "hutang" },
                              })
                            }
                ...
                <table className="w-full text-left border-collapse min-w-[1200px]">
                  <thead>
                    <tr className="bg-gray-50/80 border-b border-gray-200">
                      ...
                      <th className="py-3 px-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider border-r border-gray-200 w-56">
                        Pilih Barang
                      </th>
                      <th className="py-3 px-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider border-r border-gray-200 w-48">
                        Nama Barang Baru
                      </th>
                      ...
                      <th className="py-3 px-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider text-center w-14">
                        Aksi
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                          ...
                          <td className="py-2 px-3 border-r border-gray-100">
                            ...
                            field.selectedBarang.value === "new" ? (
                              ...
                              <span className="text-xs text-gray-400 italic bg-gray-50 px-2 py-1.5 rounded w-full block border border-gray-100 text-center">
                          ...
                          <td className="py-2 px-3 border-r border-gray-100">
                            <div className="flex bg-gray-100 rounded-lg p-1 border border-gray-200">
                              <label
                                className={`flex-1 flex flex-col items-center justify-center cursor-pointer py-1 px-2 rounded-md transition-all ${field.pembayaran === "cash" ? "bg-teal-600 text-white shadow-sm" : "text-gray-500 hover:bg-gray-200"}`}
                              >
                                <input
                                  type="radio"
                                  className="hidden"
                              ...
                              <label
                                className={`flex-1 flex flex-col items-center justify-center cursor-pointer py-1 px-2 rounded-md transition-all ${field.pembayaran === "hutang" ? "bg-red-500 text-white shadow-sm" : "text-gray-500 hover:bg-gray-200"}`}
                              >
                                <input
                                  type="radio"
                                  className="hidden"
